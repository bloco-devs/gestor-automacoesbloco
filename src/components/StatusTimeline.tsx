...
export function StatusTimeline({ current, compact = false }: { current: PipelineStatus; compact?: boolean }) {
  ...
  return (
    <ol className={cn("flex items-center", compact ? "w-full flex-nowrap gap-0.5 overflow-hidden" : "flex-wrap gap-2")}>
      {PIPELINE_ORDER.map((s, i) => {
        const done = i < currentIdx;
        const active = i === currentIdx;
        return (
          <li key={s} className={cn("flex items-center", compact ? "min-w-0 flex-none gap-0.5" : "shrink-0 gap-2")}>
            <div
              className={cn(
                "flex min-w-0 items-center rounded-full border text-xs transition-colors whitespace-nowrap",
                compact ? "gap-1 px-1.5 py-0.5 text-[10px] sm:text-xs" : "gap-2 px-3 py-1",
            ...
            {i < PIPELINE_ORDER.length - 1 && <div className={cn("h-px shrink-0 bg-border", compact ? "w-0.5 sm:w-1" : "w-3")} />}
          </li>
        );
      })}
    </ol>
